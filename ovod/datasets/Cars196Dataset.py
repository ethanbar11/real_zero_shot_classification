...
class Cars196Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.classnames = loadmat(os.path.join(self._base_folder, "cars_annos.mat"), squeeze_me=True)["class_names"]
        self.classnames = [fix_classname_special_chars(x) for x in self.classnames]
        self._split = "train" if train else "test"
        annotations = loadmat(os.path.join(self._base_folder, f"cars_{self._split}_annos.mat"), squeeze_me=True)
        # other_annotations = loadmat(os.path.join(self._base_folder,'cars_annos.mat'))['annotations']
        # other_annotations = [x[0] for x in other_annotations]
        data = [list(x.flat[0]) for x in annotations['annotations']]
        columns = ['bbox_x1', 'bbox_y1', 'bbox_x2', 'bbox_y2', 'class', 'fname']
        self.df = pd.DataFrame(data, columns=columns)
        self.df['class'] = self.df['class'] - 1  # Fix class range from 1-196 to 0-195
        self._images_folder = os.path.join(root, f"cars_{self._split}")
        ...
        self._labels = list(self.df['class'])
        self._image_files = list(self.df['fname'].apply(lambda x: os.path.join(self._images_folder, x)))
